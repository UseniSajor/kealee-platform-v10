...
const start = async () => {
  try {
    ...
    await safeRegisterBlock('OrgBot routes', async () => {
      const { orgBotsRoutes } = await import('./modules/orgbots/orgbots.routes.js')
      await fastify.register(orgBotsRoutes, { prefix: '/api/v1/orgbots' })
    })
