...
export const permitPackageService = {
  ...
  async generateCoverSheetAndIndex(
    ...
    await prismaAny.permitPackageDocument.create({
      data: {
        ...
        fileUrl: `generated://cover-sheet/${packageId}/${new Date().toISOString()}`, // Metadata placeholder - actual PDF generated by document service
    ...
    await prismaAny.permitPackageDocument.create({
      data: {
        ...
        sourceType: 'GENERATED',
        fileUrl: `generated://index/${packageId}/${new Date().toISOString()}`, // Metadata placeholder - actual PDF generated by document service
        fileName: 'index.pdf',
        fileType: 'PDF',
  ...
  async calculatePermitFees(
    ...
    if (template?.feeCalculationRules) {
      ...
      // Document fee (per sheet) - get actual sheet count from the package documents
      if (rules.documentFeePerSheet) {
        const packageDocCount = await prismaAny.permitPackageDocument.count({
          where: { packageId, documentType: 'DRAWING' },
        })
        const sheetCount = packageDocCount > 0 ? packageDocCount : 10
  ...
  async submitPermitPackage(packageId: string, data: {
    ...
    if (data.submissionMethod === 'API' && package_.jurisdictionId) {
      try {
        // Create a permit record via Prisma for the permits module to pick up
        const permitRecord = await prismaAny.permit.create({
          data: {
            projectId: package_.designProjectId,
            permitType: package_.permitType || 'BUILDING',
            status: 'SUBMITTED',
            applicationDate: new Date(),
            description: `Permit submission from package: ${package_.packageName}`,
            metadata: {
              sourceType: 'PERMIT_PACKAGE',
              packageId: package_.id,
              jurisdictionId: package_.jurisdictionId,
              submissionMethod: data.submissionMethod,
              documentCount: package_.documents.length,
            } as any,
          },
        })
        submission.apiRequestId = permitRecord.id
        submission.apiResponse = { permitId: permitRecord.id, status: 'SUBMITTED' }
  ...
  async syncPermitPackageStatus(packageId: string) {
    ...
    try {
      ...
      const submission = await prismaAny.permitPackageSubmission.findUnique({
        where: { id: package_.submissionReferenceId },
      })

      if (submission?.apiRequestId) {
        const permit = await prismaAny.permit.findUnique({
          where: { id: submission.apiRequestId },
          select: {
            status: true,
            reviewStatus: true,
            updatedAt: true,
        ...
        if (permit) {
          permitStatus = permit
          // Sync the status back to the permit package
          await prismaAny.permitPackage.update({
            where: { id: packageId },
            data: {
              reviewStatus: permit.reviewStatus || package_.reviewStatus,
              lastSyncAt: new Date(),
            },
          })
        }
      ...
      console.warn('Failed to sync permit status:', error.message)
    }

    return {
      status: permitStatus?.status || package_.status,
      reviewStatus: permitStatus?.reviewStatus || package_.reviewStatus,
      lastSyncAt: new Date(),
    }
  },
