...
# Add the basement areaway shown in Kingsworth, retaining the selected footprint.
# Lot 2 needs a 0.7-foot shift toward the frontage to clear the areaway at 8 ft.
MOVE_FT=.7
h=list(houses[1].exterior.coords)[:-1]
rear=sorted(h,key=lambda a:a[0])[:2]
a=max(rear,key=lambda a:a[1]);b=min(rear,key=lambda a:a[1])
u=((b[0]-a[0])/math.dist(a,b),(b[1]-a[1])/math.dist(a,b))
front=(-u[1],u[0]);dx,dy=front[0]*MOVE_FT*SCALE,front[1]*MOVE_FT*SCALE
original_houses=list(houses)
houses[1]=translate(houses[1],dx,dy)
# Translate just the complete Lot 2 building render block in the original stream.
stream_xref=p.get_contents()[0];stream=doc.xref_stream(stream_xref).decode('latin1')
start=stream.index('1340.29 710.88 m\n1310.08 548.06 l')
end=stream.index('540.9 525.23 m\n347.74 248.55 l',start)
stream=stream[:start]+f'q\n1 0 0 1 {dx:.8f} {dy:.8f} cm\n'+stream[start:end]+'Q\n'+stream[end:]
# Shift the existing 8 x 4 stoop polygon (fill, clip and outline) with the house.
stoop_old=shape(drawings[1026]);stoop_new=translate(stoop_old,dx,dy)
oldpts=list(stoop_old.exterior.coords);newpts=list(stoop_new.exterior.coords)
def path_token(point):return f'{point[0]:.2f}'.rstrip('0').rstrip('.')+' '+f'{point[1]:.2f}'.rstrip('0').rstrip('.')
for a,b in zip(oldpts[:-1],newpts[:-1]):
 stream=stream.replace(path_token(a)+' m',path_token(b)+' m').replace(path_token(a)+' l',path_token(b)+' l')
# Remove the private corridor rectangle and its hatch, keeping the recorded
# lower route as a line without asserting an unlettered private legal width.
private_path='747.45 384.63 m\n993.38 576.98 l\n1015.56 548.62 l\n794.67 375.87 l\n747.45 384.63 l\nh\n'
for _ in range(2):
 a=stream.index('q\n'+private_path);b=stream.index('\nQ\n',a)+3;stream=stream[:a]+stream[b:]
a=stream.index(private_path);b=stream.index('\nS\n',a)+3;stream=stream[:a]+stream[b:]
doc.update_stream(stream_xref,stream.encode('latin1'))
areaways=[];areaway_checks=[]
for i,house in enumerate(houses):
 h=list(house.exterior.coords)[:-1];rear=sorted(h,key=lambda a:a[0])[:2]
 a=max(rear,key=lambda a:a[1]);b=min(rear,key=lambda a:a[1]);u=((b[0]-a[0])/math.dist(a,b),(b[1]-a[1])/math.dist(a,b));n=(u[1],-u[0])
 r=[a,(a[0]+u[0]*13*SCALE,a[1]+u[1]*13*SCALE),(a[0]+u[0]*13*SCALE+n[0]*4*SCALE,a[1]+u[1]*13*SCALE+n[1]*4*SCALE),(a[0]+n[0]*4*SCALE,a[1]+n[1]*4*SCALE)]
 g=Polygon(r);gap=g.distance(lots[i].boundary)/SCALE
 assert lots[i].covers(g) and gap>=8, (i,gap)
 areaways.append(g);areaway_checks.append({'lot':i+1,'areaSF':g.area/SCALE**2,'minimumBoundaryGapFt':gap,'pass':True})
...
for d in drawings:
 ...
 if col in [(0.725,0.725,0.725),(.937,.937,.937),(.886,.886,.886),(.827,.827,.827),(.867,.867,.867)]:
  try:pavements.append(stoop_new if d is drawings[1026] else shape(d))
...
lod=unary_union(lots+pavements+areaways+[public]);areas=[9598,8008]
measured=[]
...
for block in p.get_text('dict')['blocks']:
 for line in block.get('lines',[]):
  for span in line['spans']:
   ...
   if 'PRIVATE UTILITY' in old or old=='LOT 2, OVER LOT 1':new=''
...
# Draw the source areaways and the shortened private route after redactions.
for i,g in enumerate(areaways):
 p.draw_polyline([pdf.Point(a) for a in g.exterior.coords],color=(.35,.35,.35),fill=(.93,.93,.93),width=1,closePath=True)
 xx,yy=g.centroid.coords[0];text(p,xx-92,yy+8,f'AW-{i+1} 13 x 4 ft',9,True)
# The existing public tip defines the existing draft junction. This correction
# removes only the unsupported upper private extension, not legalizes that tip.
C=nearest_points(public,lots[0].boundary)[1];V=Point(993.39,576.985)
p.draw_line(pdf.Point(C.coords[0]),pdf.Point(V.coords[0]),color=(.416,.106,.604),width=1.2,dashes='[12 3 2 3]')
xx,yy=LineString([C.coords[0],V.coords[0]]).interpolate(.5,normalized=True).coords[0]
text(p,xx-70,yy+28,'PRIVATE UTILITY EASEMENT',9)
# Provide a continuous five-foot leadwalk from each depicted stoop to driveway.
walk_checks=[];walks=[]
for i,(stoop,drive) in enumerate([(shape(drawings[627]),shape(drawings[521])),(stoop_new,shape(drawings[920]))]):
 a,b=nearest_points(stoop,drive);g=LineString([a,b]).buffer(2.5*SCALE,cap_style=2).intersection(lots[i]).difference(houses[i])
 assert g.geom_type=='Polygon' and g.distance(stoop)<.03 and g.distance(drive)<.03
 p.draw_polyline([pdf.Point(a) for a in g.exterior.coords],color=(.45,.45,.45),fill=(.88,.88,.88),width=.7,closePath=True)
 walks.append(g)
 walk_checks.append({'lot':i+1,'connectsStoopAndDrive':True})
# Partition the actual selected geometry; count every overlap only once.
all_hard=unary_union(houses+areaways+pavements+walks)
quantity=[];quantity_shapes=[]
for i,lot in enumerate(lots):
 assigned=Polygon();parts={}
 for label,geometry in [('Dwelling footprint',houses[i]),('Areaway',areaways[i]),('Baseline paving',pave),('Added leadwalk',walks[i])]:
  part=geometry.intersection(lot).difference(assigned)
  parts[label]=part.area/SCALE**2
  quantity_shapes.append((i,label,part))
  assigned=unary_union([assigned,part])
 impervious=all_hard.intersection(lot).area/SCALE**2
 assert abs(sum(parts.values())-impervious)<.01
 percent=100*impervious/areas[i];rv=.05+.009*percent
 assert percent>15
 quantity.append({'lot':i+1,'recordAreaSF':areas[i],'surfacesSF':parts,'imperviousSF':impervious,'imperviousPct':percent,'remainingAreaSF':areas[i]-impervious,'rv':rv,'screeningRainfallIn':1.0,'screeningWQvCF':rv*areas[i]/12})
offsite_hard=all_hard.difference(lot_union).area/SCALE**2
(ROOT/'surface-takeoff.json').write_text(json.dumps({'lots':quantity,'offsiteDepictedHardSurfaceSF':offsite_hard,'basis':'Selected drawing geometry; lot-wide screening, not delineated facility catchments','peakDischarges':None},indent=2)+'\n')
with (ROOT/'surface-takeoff.csv').open('w',newline='') as f:
 writer=csv.writer(f);writer.writerow(['Lot','Surface','Area SF'])
 for row in quantity:
  for label,area in row['surfacesSF'].items():writer.writerow([row['lot'],label,f'{area:.3f}'])
  writer.writerow([row['lot'],'Total impervious',f'{row["imperviousSF"]:.3f}'])
 writer.writerow(['Outside both lots','Depicted hard surface',f'{offsite_hard:.3f}'])
draw_geom(p,lod)
...
for s in ['1005 & 1009 ROLLINS AVENUE','PRINCE GEORGE\'S COUNTY, MARYLAND','C-001  |  SITE DEVELOPMENT / FINE GRADING','REVISION 3 - 10 SEPTEMBER 2026','PRELIMINARY - NOT FOR CONSTRUCTION']:
...
notes=[
 ...
 'Water and sanitary routing is preliminary. Recorded easements are the project basis. Coordinate the WSSC junction geometry, service separation and profiles. County details and the source connection sketch appear on this sheet.',
 'The 46 x 26 dwelling footprint includes the 10-foot one-car garage. Rear basement areaways: 13 x 4 ft per Kingsworth. Lot 2 is shifted 0.70 ft toward Rollins Avenue. Coordinate basement plumbing and final veneer projections.',
]
...
assert y<1695,y
# Updated screening quantities occupy the space above the technical details.
p.draw_rect(pdf.Rect(1035,1110,2020,1298),color=None,fill=(1,1,1))
z=heading(p,1050,1140,930,'UPDATED SURFACE TAKEOFF / LOT-WIDE WATER-QUALITY SCREENING')
z=table(p,1050,z,930,['ITEM','LOT 1','LOT 2'],[
 ['Depicted impervious area',*[f'{r["imperviousSF"]:,.1f} SF' for r in quantity]],
 ['Impervious / recorded lot',*[f'{r["imperviousPct"]:.2f}%' for r in quantity]],
 ['WQv screening, P = 1 inch',*[f'{r["screeningWQvCF"]:.1f} CF' for r in quantity]]])
text(p,1050,z,'Lot-wide screening only; see dossier page 7 for assumptions and calculation basis. Final tributary and ESD design pending.',10)
...
main=ROOT/'Rollins-Avenue-Record-Coordinated-Site-Plan.pdf';doc.set_metadata({'title':'Rollins Avenue - selected baseline revision','subject':'Preliminary civil drawing; unresolved design items in supporting dossier'})
...
def new(title):
 ...
 text(q,36,770,f'PRELIMINARY COORDINATION | 10 SEPTEMBER 2026 | {len(support)}',9)
 return q,90
...
y=para(q,36,y,540,'Quantities use the selected PDF geometry at 3.6 points per foot, with the documented 0.70-ft Lot 2 translation applied. Both dwelling footprints remain 1,196 SF. The areaways add 52 SF each outside those nominal footprints.')
...
y=para(q,36,y,540,'The selected PDF shows both nominal 46 x 26 dwellings. Kingsworth basement areaways have been added, 13 x 4 ft each. Lot 2 moves 0.70 ft toward Rollins Avenue to maintain the current 8-ft side design criterion at the areaway while keeping the dwelling front beyond 30 ft. Basement vertical design, veneer and roof projections still require coordination.')
...
issues=[
 ('R01','PARTIAL','Recorded boundary survey and WSSC benchmarks documented; vertical datum, grading, floor and utility design remain'),
 ...
 ('R04','PARTIAL','Recorded status confirmed by applicant; upper private extension removed; no unlettered width asserted'),
 ('R05','PARTIAL','Updated surface takeoff and lot-wide WQv screening supplied; approved concept, catchments and facility design remain'),
 ...
 ('R12','PARTIAL','Kingsworth areaways added; Lot 2 shifted to clear 8-foot side; complete upper building/trade designs remain'),
...
q,y=new('Updated surface takeoff and water-quality screening')
y=para(q,36,y,540,'Revision 3 supersedes the baseline drainage summary. Quantities include the shifted dwelling/stoop, both areaways and new leadwalks. Surface overlaps are counted once and clipped to each lot. Recorded lot areas provide the percentage denominator.')
rows=[]
for label in quantity[0]['surfacesSF']:
 rows.append([label,*[f'{r["surfacesSF"][label]:,.2f}' for r in quantity]])
rows.extend([
 ['Total impervious SF',*[f'{r["imperviousSF"]:,.2f}' for r in quantity]],
 ['Impervious percent',*[f'{r["imperviousPct"]:.2f}%' for r in quantity]],
 ['Rv',*[f'{r["rv"]:.5f}' for r in quantity]],
 ['Screening WQv CF',*[f'{r["screeningWQvCF"]:.2f}' for r in quantity]]])
y=table(q,36,y,540,['PARAMETER','LOT 1','LOT 2'],rows)
y=para(q,36,y,540,'Calculation: I = 100 x impervious SF / recorded lot SF; Rv = 0.05 + 0.009 x I; WQv (CF) = P (in) x Rv x lot area (SF) / 12. P = 1.0 inch is the screening assumption retained from the baseline. Both lots exceed 15% impervious cover.',10)
y=para(q,36,y,540,'Method source: Maryland Stormwater Design Manual, Chapter 2, Section 2.1, page 2.2; checked September 10, 2026. Official source link is in README.md. These lot-wide calculations do not establish facility tributaries, required ESD treatment depth or available storage.',10)
y=para(q,36,y,540,f'Depicted hard surface outside both lots: {offsite_hard:,.2f} SF, reported separately. Its drainage destination and treatment allocation require grading design. No portion is silently assigned to either lot facility.',10)
y=para(q,36,y,540,'Dwelling footprint is a roof proxy pending roof/eave plans. Areaways are treated as impervious. Paving includes all depicted hard surfaces within each lot. Leadwalk quantities are incremental, excluding overlap with other surfaces. Unshown improvements are excluded. Lot boundaries are screening areas, not surveyed drainage divides.',10)
y=para(q,36,y,540,'Previous peak discharge figures are withdrawn from this summary. Recompute with documented existing cover, flow paths, time of concentration, rainfall intensity and final grading. Facility sections, infiltration, groundwater, outlets and overflow remain required; footprint area alone establishes no storage capacity.',10)
q,y=new('WSSC record benchmarks and utility evidence')
y=para(q,36,y,540,'The supplied WSSC final as-built sheet contains record benchmark elevations, existing utility profiles and adjoining-lot connection annotations. These values were omitted from the earlier source inventory. The applicant confirms that the supplied plats/easements are the recorded project documents.')
y=table(q,36,y,540,['BENCHMARK','RECORD ELEVATION'],[['BM 1','213.89 ft'],['BM 2','218.28 ft'],['BM 3','214.61 ft']])
y=para(q,36,y,540,'BM 1: PK nail in west side of Pepco pole 826379-0261, east side of Rollins Avenue, approximately 700 ft south of District Avenue. BM 2: PK nail in west side of Pepco pole, east side of Rollins Avenue, approximately 500 ft south of District Avenue. BM 3: high point in concrete monument, approximately 48 ft northeast near the right corner of house 5912 and 46 ft northeast near the left corner of house 5910. Verify identification against the original image before field use.',10)
y=para(q,36,y,540,'These are historical WSSC record elevations, not newly observed benchmark recoveries. The scan does not expressly establish their relationship to the NAVD88 contour data used on C-001. Profile labels DATUM 190 / 200 identify plot baselines and are not a vertical-datum transformation. No datum offset has been guessed.',10)
y=heading(q,36,y+8,540,'EASEMENT AND BASEMENT COORDINATION')
y=para(q,36,y,540,'Use the supplied recorded easement documents as the project basis. Page 2 shows a 20-ft public WSSC corridor on Lot 13 and the private route on the lower portion of Lot 1. The source does not dimension the private width; the revised site sheet therefore shows the private route without the earlier rectangular width assumption. Public-junction geometry and service continuity remain to be reconciled.',10)
y=para(q,36,y,540,'The foundation source supplies basement/areaway geometry, not site-specific slab or service elevations. Basement plumbing and areaway drainage must be checked against the applicable WSSC sewer profile and a consistent vertical datum before selecting gravity connections or a pump design.',10)
q,y=new('Kingsworth basement/areaway source')
arch=pdf.open(REPO/'existing site plans/Kingsworth 2nd & basement floor plan.pdf')
q.show_pdf_page(pdf.Rect(28,110,584,570),arch,0)
para(q,36,610,540,'Original foundation/basement drawing retained as source evidence. Site areaways AW-1 and AW-2 follow its 13-ft overall length and 4-ft depth. The original author\'s notes apply to this source document; this reproduction does not constitute new professional approval.',10)
q,y=new('Surface takeoff exhibit - lot screening boundaries')
y=para(q,36,y,540,'This exhibit identifies measured hard surfaces and the lot boundaries used for the screening on page 7. It is not a proposed grading or facility drainage-area map.',10)
colors={'Dwelling footprint':(.75,.80,.86),'Areaway':(.91,.65,.34),'Baseline paving':(.65,.65,.65),'Added leadwalk':(.39,.72,.54)}
# Uniform scale, with PDF north orientation unchanged.
bounds=lot_union.bounds;k=min(500/(bounds[2]-bounds[0]),440/(bounds[3]-bounds[1]))
def exhibit_point(a):return pdf.Point(52+(a[0]-bounds[0])*k,160+(a[1]-bounds[1])*k)
def exhibit_geom(g,color,fill):
 if g.is_empty:return
 if hasattr(g,'geoms'):
  for part in g.geoms:exhibit_geom(part,color,fill)
 elif g.geom_type=='Polygon':
  sh=q.new_shape();sh.draw_polyline([exhibit_point(a) for a in g.exterior.coords])
  for ring in g.interiors:sh.draw_polyline([exhibit_point(a) for a in ring.coords])
  sh.finish(color=color,fill=fill,width=.6,closePath=True,even_odd=True);sh.commit()
for lot in lots:exhibit_geom(lot,(.1,.1,.1),None)
for i,label,g in quantity_shapes:exhibit_geom(g,(.2,.2,.2),colors[label])
for i,lot in enumerate(lots):
 a=exhibit_point(lot.representative_point().coords[0]);text(q,a.x-35,a.y,f'LOT {i+1}',12,True)
text(q,550,147,'N',11,True);q.draw_line((554,182),(554,152),color=INK);q.draw_line((554,152),(550,160),color=INK);q.draw_line((554,152),(558,160),color=INK)
y=630
for label,col in colors.items():
 q.draw_rect(pdf.Rect(36,y-10,52,y+2),fill=col,color=INK,width=.4);text(q,62,y,label,10);y+=20
para(q,36,725,540,'Areas use the full-size source geometry. Display exhibit not to scale. Roof overhangs, final tributaries and offsite drainage allocation remain to be coordinated.',9)
...
checks={'baselineSHA256':HASH,'sitePlanPages':len(doc),'supportingPages':len(support),'submissionReady':False,'measuredFromSelectedPDFWithLot2Shift':measured,'lot2ShiftTowardStreetFt':MOVE_FT,'lot2ShiftPdfPoints':[dx,dy],'areawayChecks':areaway_checks,'walkChecks':walk_checks,'surfaceTakeoff':quantity,'offsiteHardSurfaceSF':offsite_hard,'geometry':{'houses':[list(g.exterior.coords) for g in houses],'areaways':[list(g.exterior.coords) for g in areaways],'lot2Stoop':list(stoop_new.exterior.coords),'walks':[list(g.exterior.coords) for g in walks]},'proposedLODsf':lod.area/SCALE**2,'offsiteLODsf':outside,'issues':[dict(zip(['id','status','resolution'],r)) for r in issues]}
