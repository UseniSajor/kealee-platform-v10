...
REV = ROOT/'revision-2'; REV.mkdir(exist_ok=True)
OUT = REV/'eplan'; OUT.mkdir(exist_ok=True)
...
# Labels transcribed visually from the recorded plat, not rounded from geometry.
# Geometry is retained pending survey reconciliation of the small discrepancies.
boundary_calls=[
    ('B1',0,0,'N 79-29-14 E',176.36),
    ('B2',0,1,'S 10-30-46 E',65.00),
    ('B3',0,2,'N 79-29-14 E',118.96),
    ('B4',0,3,'N 51-58-25 W',86.72),
    ('B5',1,0,'S 10-30-46 E',134.64),
    ('B6',1,1,'N 51-58-25 W',179.66),
]
readiness=[]
for route in routes:
    if not route['id'].startswith('public-'):continue
    lot=route['lot'];water=route['water']
    house_service=next(f for f in F.values() if f['kind']=='Utility' and f['id'].startswith(f'l{lot}-') and (('Water' in f['attributes']['type'])==water))
    downstream=[LineString(house_service['line'])]
    if lot==2:downstream += [LineString(r['line']) for r in routes if r['id'].startswith('private-') and r['water']==water]
    gap=Point(route['line'][-1]).distance(unary_union(downstream))
    readiness.append({'check':f'Lot {lot} {"water" if water else "sewer"} public-to-service junction', 'pass':gap<=.02,'gapFt':round(gap,3),'resolution':'Surveyed WSSC junction design; no connecting geometry inferred'})
for i in (1,2):
    swm=poly(next(f for f in F.values() if f['kind']=='SWMPractice' and f['id'].startswith(f'l{i}-')))
    obstacles=unary_union([houses[i-1]]+[poly(F[f'l{i}-{suffix}']) for suffix in ['walk','stoop','driveway']])
    checks.append({'check':f'Lot {i} SWM footprint clears dwelling, walk, stoop and driveway', 'pass':not swm.intersects(obstacles),'minimumGapFt':round(swm.distance(obstacles),3)})
    checks.append({'check':f'Lot {i} driveway meets apron','pass':poly(F[f'l{i}-driveway']).distance(poly(F[f'l{i}-apron']))<.02})
    curbs=unary_union([poly(f) for f in F.values() if f['id'].startswith('frontage-curb-')])
    checks.append({'check':f'Lot {i} apron meets curb','pass':poly(F[f'l{i}-apron']).distance(curbs)<.02})
    public_walks=unary_union([poly(f) for f in F.values() if f['id'].startswith('frontage-sidewalk-')])
    checks.append({'check':f'Lot {i} SWM clears public sidewalk','pass':not swm.intersects(public_walks),'minimumGapFt':round(swm.distance(public_walks),3)})
...
def frame(doc,number,title):
    ...
    text(p,1550,HEIGHT-44,'REVISION 2 | 09 SEPTEMBER 2026 | PRELIMINARY - PROFESSIONAL REVIEW',11)
...
def plot(p,rect,esc=False):
    ...
    def service(line, water):
        g=LineString(line); color=BLUE if water else GREEN
        draw_geo(p,g,tr,color,1.15,dashes='[12 4]' if water else '[14 3 2 3]')
        # Letter tags distinguish services in monochrome reproductions too.
        for distance in range(15 if water else 30,int(g.length),35):
            q=g.interpolate(distance); xx,yy=tr(q.coords[0])
            text(p,xx+3,yy-3,'W' if water else 'S',9,True,color)
    for route in routes:service(route['line'],route['water'])
    for f in F.values():
        if f['kind']=='Utility' and f['id'].startswith(('l1-','l2-')):
            service(f['line'],'Water' in f['attributes']['type'])
    # Boundary references link each drawn line to its recorded call schedule.
    for ident,li,edge,bearing,length in boundary_calls:
        ring=list(lots[li].exterior.coords); edge_line=LineString([ring[edge],ring[edge+1]])
        mid=edge_line.interpolate(.5,normalized=True); xx,yy=tr(mid.coords[0])
        text(p,xx-55 if ident=='B4' else xx+10,yy+30 if ident=='B4' else yy-13,ident,12,True)
...
for s in [
    'Two detached dwellings. Boundary control: recorded Porter Subdivision plat, PM 231 / 50. Existing contours: 2-foot interval, NAVD88. Proposed dwelling footprints: 46 ft x 26 ft, including the optional one-car garage.',
...
y=para(p,x,y,w,'Both lots: water and sanitary connections toward the existing 8-inch mains in Modupeola Way, through the 20-foot WSSC corridor on Lot 13. Private utility segment runs from the Lot 1 / 12 / 13 corner toward the Lot 1 / 2 / 12 corner. ')
y=para(p,x,y,w,'WSSC connection reference: C-004. Utility routing is preliminary; junctions, fittings, separation, depths and main tie-ins require coordinated WSSC design. Private easement limits are governed by the recorded instrument.')
y=heading(p,x,y+10,'LEGEND',w)
for color,label,dash in [(INK,'Recorded lot line',None),(GREY,'Building restriction line','[7 4]'),(PURPLE,'Utility easement','[10 3 2 3]'),(BLUE,'W - Water house connection','[12 4]'),(GREEN,'S - Sanitary house connection','[14 3 2 3]')]:
...
y=heading(p,x,y+14,'BOUNDARY CALLS OF RECORD - PM 231 / 50',w)
y=table(p,x,y,w,['LINE','BEARING','DISTANCE'],[[a[0],a[3],f'{a[4]:.2f} ft'] for a in boundary_calls])
y=para(p,x,y,w,'B3 is the common Lot 1 / Lot 2 boundary. Bearings are stated in the direction lettered on the recorded plat. PUE: 10 ft along each Rollins frontage; public dedication: 20 ft.')
...
y=para(p,x,y,w,'The recorded plat supplies the boundary survey/control for this drawing. Original owner/surveyor signatures apply to that recorded instrument. Horizontal control: NAD83. Existing contour interval: 2 ft. Field control and proposed grades require survey and civil design coordination.',12)
...
for s in ['Q = C i A; A in acres. Weighted C uses roof/paving 0.95 and pervious area 0.25. Impervious areas use geometric unions of the dwelling, driveway, stoop and corrected leadwalk.',
    ...
    'Frontage paving is excluded from lot catchments and listed in the disturbance takeoff. Final drainage analysis must include its receiving system and all upstream tributary areas. ']:
    y=para(p,x,y,w,s,12)
...
if soil_path.exists():
    ...
    if soil_rows:
        ...
        y=para(p,x,y,w,'Soil map-unit properties are regional mapping data. Facility design requires site-specific infiltration and groundwater investigation.',11)
...
para(p,1690,1220,800,'WATER AND SEWER CONNECTION SKETCH - SOURCE REFERENCE. Page 2 of the supplied WSSC connection document. See C-001 for the preliminary utility layout.',13)
...
target=REV/'Rollins-Avenue-Civil-Review-Rev2.pdf'
...
for i,p in enumerate(doc):
    ...
    p.get_pixmap(matrix=pdf.Matrix(.65,.65)).save(str(REV/f'preview-{i+1}.png'))
data={'sourceBestBackupSHA256':hashlib.sha256(BASE.read_bytes()).hexdigest(),'status':'CORRECTED_DRAWINGS_PROFESSIONAL_REVIEW_REQUIRED','pages':len(doc),'recordAreas':record_areas,'disturbanceUnionSqFt':lod_area,'clearances':distances,'drainage':drain,'geometryChecks':checks,'submissionReady':False,'utilityConnectivityChecks':readiness,'boundaryCallsOfRecord':[{'id':a[0],'bearing':a[3],'distanceFt':a[4]} for a in boundary_calls],'remaining':['Public-to-private utility junction continuity and surveyed easement reconciliation','Professional signatures and certification','Final vertical grading, utility/infiltration design values not present in records','Current approval attachments and private easement legal instrument','Lot 2 zoning interpretation and DPW&T driveway/sidewalk determination','Complete coordinated architectural and structural building set']}
(REV/'verification.json').write_text(json.dumps(data,indent=2)+'\n')
(REV/'corrected-features.json').write_text(json.dumps({'features':list(F.values()),'utilityRoutes':routes,'lodWkt':lod.wkt,'publicCorridorWkt':public.wkt,'privateWorkingCorridorWkt':private.wkt},indent=2)+'\n')
assert len(doc)==4
assert all(c['pass'] for c in checks),checks
