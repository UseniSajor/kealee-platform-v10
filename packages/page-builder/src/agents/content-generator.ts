...
export async function generateHeroContent(
  ...
  try {
    const response = await ai.reason({
      task: 'Generate a personalized hero section for an powered by AI tools construction concept page. Return valid JSON only.',
      context: {
      ...
      schema: {
        headline: 'string — compelling headline emphasizing design concept/3D visualization (max 80 chars)',
      ...
      systemPrompt:
        'You are a marketing copywriter for Kealee, a construction project management platform. Write compelling headlines for users who want to see their dream project in 3D with generated using AI tools concepts. Focus on Concept Packages starting at $99. Do NOT mention contractors or contractor matching — users see design concepts first, then architecture, then permits, then construction. Return ONLY valid JSON, no markdown.',
    })
    ...
    return {
      ...
      ctaText: parsed.ctaText || 'Get Your design concept',
