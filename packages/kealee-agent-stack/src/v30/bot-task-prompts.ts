...
export function buildV30BotUserPrompt(botType: V30BotType, inputData: Record<string, unknown>): string {
  ...
  const tasks: Record<V30BotType, string> = {
    ...
    zoning: `Produce ZoningBot output at the requested property-intelligence depth (${String(inputData.propertyIntelligenceDepth ?? 'project')}): jurisdiction, verification date, confidence, source links, likely versus confirmed requirements, requiredPermits, documentsChecklist, fees, timelines, and filing responsibility. Never represent preliminary research as agency confirmation or permit approval. ${jsonOnly}\n\n${ctx}`,
    ...
    permit: `Produce a PRELIMINARY PermitBot roadmap: likely specification sections, submission checklist, verification needs, filing responsibility, and qualified-professional requirements. Never call AI output permit-ready, stamped, sealed, approved, certified, code-compliant, construction-ready, or suitable for filing. Distinguish assumptions from confirmed jurisdiction facts and require human verification. ${jsonOnly}\n\n${ctx}`,
