/**
 * @kealee/agent-utils/scoring/contractor
 *
 * Contractor matching score using weighted multi-factor algorithm + haversine distance.
 *
 * Weights: tradeMatch 35%, location 20%, experience 20%, rating 15%, availability 10%
 *
 * Source: extracted from bots/keabot-contractor-match/src/scoring.ts
 * Pure function — no I/O, no LLM, no database.
 */

import type { Contractor, ContractorScore, MatchQuery } from './contractor-types';
...
/**
 * Calculate great-circle distance between two lat/lng coordinates in kilometres.
 */
...
/**
 * Filter, score, and rank contractors for a given match query.
 * Returns top 10 results sorted by descending score.
 */
export function rankContractors(
  contractors: Contractor[],
  query: MatchQuery,
): Array<{ contractor: Contractor; score: ContractorScore }> {
  const radiusKm = query.radiusKm ?? 25;
  return contractors
