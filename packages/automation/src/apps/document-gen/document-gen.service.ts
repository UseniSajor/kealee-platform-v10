...
export class DocumentGeneratorService {
  ...
  async generateDocument(opts: GenerateDocumentOpts): Promise<{
    ...
    const tenantBranding = project.orgId
      ? await (prisma as any).whiteLabelTenantProfile.findFirst({
          where: { orgId: project.orgId, status: 'ACTIVE', org: { status: 'ACTIVE', tenantKind: 'WHITE_LABEL' } },
          include: {
            org: {
              select: {
                whiteLabelDomains: {
    ...
    const allVariables: Record<string, any> = {
      ...
      brand_company_name: tenantBranding?.companyName ?? 'Kealee',
    ...
    const fullHtml = this.wrapInHtmlPage(
      ...
      tenantBranding ? {
        ...
        productName: tenantBranding.productName,
        ...
        supportName: tenantBranding.supportName,
        supportEmail: tenantBranding.supportEmail,
        supportPhone: tenantBranding.supportPhone,
        supportUrl: tenantBranding.supportUrl,
        reportHeader: tenantBranding.reportHeader,
        reportFooter: tenantBranding.reportFooter,
        legalDisclaimer: tenantBranding.legalDisclaimer,
        kealeeBrandingVisible: tenantBranding.kealeeBrandingVisible,
      } : undefined,
    );

    // 6. In a production system, convert HTML → PDF and upload to storage.
    //    For now, store the HTML content and simulate a file URL.
    const tenantHostname = tenantBranding?.org?.whiteLabelDomains?.[0]?.hostname;
    const documentBaseUrl = tenantHostname
      ? `https://${tenantHostname}`
      : (process.env.PUBLIC_DOCUMENT_BASE_URL ?? 'https://storage.kealee.com');
    const simulatedFileUrl = `${documentBaseUrl}/documents/${opts.projectId}/${Date.now()}-${opts.templateType}.pdf`;
  ...
  private wrapInHtmlPage(title: string, bodyHtml: string, branding?: TenantOutboundBrandingInput): string {
    const chrome = renderBrandedDocumentChrome({ title, branding });
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
<body>
  ${chrome.headerHtml}
  ${bodyHtml}
  <div class="footer">${chrome.footerHtml}</div>
</body>
</html>`;
  }
...
<p>Standard ${variables.brand_company_name ?? 'service provider'} contract terms apply.</p>
