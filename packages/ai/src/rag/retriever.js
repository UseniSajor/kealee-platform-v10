...
async function retrieveContext(opts) {
    ...
    const queryEmbedding = await (0, vector_store_js_1.generateEmbedding)(query);
    ...
    const rows = await prisma_helper_js_1.prismaAny.$queryRawUnsafe(`
    WITH query_vec AS (
...
function formatContext(results) {
    if (results.length === 0)
        return '';
    return results
        .map((r, i) => `[Context ${i + 1} — ${r.documentTitle} (similarity: ${(r.similarity * 100).toFixed(0)}%)]\n${r.content}`)
        .join('\n\n---\n\n');
}
