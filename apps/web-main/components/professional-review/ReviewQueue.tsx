 ...
 * OS Engineering (site plans); the architect's as OS Architecture (design
 * concepts and the dwelling on the site plan). A profile registered in the
 * other discipline is told which desk is theirs.
 */
export async function ReviewQueue({ discipline, children }: { discipline: ReviewDiscipline; children?: React.ReactNode }) {
    ...
    return (
      <main className="mx-auto max-w-2xl px-6 py-12">
        <div className="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
          <p className="text-sm font-semibold uppercase tracking-wider text-emerald-700">{cfg.desk} · professional onboarding</p>
          ...
          <form action={createProfessionalProfile} className="mt-8 grid gap-5 sm:grid-cols-2">
            <input type="hidden" name="discipline" value={cfg.id} />
            <Field name="displayName" label="Professional name" required />
            <Field name="firmName" label="Firm" />
            <Field name="licenseNumber" label={cfg.licenceLabel} required />
  ...
  if (identity.discipline !== discipline) {
    const theirs = DISCIPLINES[identity.discipline ?? 'professional_engineer']
    return (
      <main className="mx-auto max-w-2xl px-6 py-12">
        <div className="rounded-2xl border border-amber-200 bg-amber-50 p-8 text-amber-950">
          <p className="text-sm font-semibold uppercase tracking-wider">{cfg.desk}</p>
          <h1 className="mt-2 text-2xl font-semibold">This desk is for {discipline.replaceAll('_', ' ')}s</h1>
          <p className="mt-3">Your profile is registered as {String(identity.discipline).replaceAll('_', ' ')}. Your queue is <Link href={theirs.path} className="font-semibold underline">{theirs.desk}</Link>.</p>
        </div>
      </main>
    )
  }
  ...
  const [assignments, allActiveAssignments] = await Promise.all([
    reviewDb.sitePlanReviewAssignment.findMany({
      where: { professionalProfileId: identity.profile.id, discipline },
      orderBy: { updatedAt: 'desc' },
    }),
    // Claimed in THIS discipline. The engineer's claim does not take a plan off the architect's list.
    reviewDb.sitePlanReviewAssignment.findMany({
      where: { discipline, status: { in: ['ACTIVE', 'REVISION_REQUIRED', 'COMPLETED'] } },
  ...
  return (
    <main className="mx-auto max-w-7xl space-y-10 px-6 py-10">
      <section className="flex flex-col justify-between gap-5 md:flex-row md:items-end">
        <div>
          <p className="text-sm font-semibold uppercase tracking-wider text-emerald-700">{cfg.desk} · professional review</p>
      ...
      <section>
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          {assignments.map((assignment: any) => {
            ...
            return (
              <Link key={assignment.id} href={`${cfg.path}/${workflow.id}`} className="group rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition hover:border-emerald-400">
      ...
      {identity.profile.isLicensed && (
        <section>
          <h2 className="mb-4 text-xl font-semibold">Site plans available for {discipline.replaceAll('_', ' ')} review</h2>
