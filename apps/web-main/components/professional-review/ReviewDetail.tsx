...
import { completeReview, recordScopedDecision } from '@/lib/professional-review-actions'
import { DISCIPLINES, displayProject, requireAssignedReview, reviewDb, type ReviewDiscipline } from '@/lib/professional-review'

/** One assigned review, for the discipline whose queue rendered it. */
export async function ReviewDetail({ workflowId, discipline }: { workflowId: string; discipline: ReviewDiscipline }) {
  const cfg = DISCIPLINES[discipline]
  let identity
  try { identity = await requireAssignedReview(workflowId, discipline) } catch { notFound() }
  ...
  const mineApprovals = approvals.filter((item: any) => item.discipline === discipline && !item.supersededById)
  const allApproved = mineApprovals.length > 0 && mineApprovals.every((item: any) =>
  ...
  return (
    <main className="mx-auto max-w-7xl space-y-8 px-6 py-8">
      <Link href={cfg.path} className="inline-flex items-center gap-2 text-sm font-medium text-slate-600 hover:text-slate-950"><ArrowLeft className="h-4 w-4" /> Back to queue</Link>

      <section className="rounded-2xl bg-slate-950 p-7 text-white">
        <div className="flex flex-col justify-between gap-6 md:flex-row md:items-end">
          <div>
            <p className="text-sm font-semibold uppercase tracking-wider text-emerald-400">{cfg.desk} · assigned {discipline.replaceAll('_', ' ')} review</p>
        ...
        <div className="space-y-8">
          ...
          <section>
            ...
            <div className="space-y-4">
              {approvals.filter((a: any) => !a.supersededById).map((approval: any) => (
                <form key={approval.id} action={recordScopedDecision} className="rounded-xl border border-slate-200 bg-white p-5">
                  ...
                  {approval.discipline === discipline ? (
