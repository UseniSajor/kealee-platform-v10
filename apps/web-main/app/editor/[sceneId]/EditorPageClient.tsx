...
export default function EditorPageClient({ sceneId, initialScene, sceneName, projectType, initialMode, initialSelectedRenderUrl }: Props) {
        ...
        {sidePanel === 'renders' && (
          <div className="w-96 border-l border-slate-200 bg-white overflow-y-auto flex-shrink-0">
            <RenderPanel
              sceneId={sceneId}
              projectType={projectType as ProjectType}
              initialSelectedRenderUrl={initialSelectedRenderUrl}
            />
          </div>
        )}
        {sidePanel === 'reels' && (
