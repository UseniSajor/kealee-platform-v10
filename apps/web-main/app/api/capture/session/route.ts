...
export async function POST(req: NextRequest) {
  try {
    ...
    const { intake_id, project_id } = parsed.data
    const project_path = normalizeCaptureProjectPath(parsed.data.project_path ?? '')
    ...
    if (!isSiteVisit && requiredZones.length === 0) {
      return NextResponse.json(
        { error: `Capture is not configured for project path "${parsed.data.project_path}"` },
        { status: 400 },
      )
    }
