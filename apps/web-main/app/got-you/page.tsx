...
import { CheckCircle2, AlertTriangle, Mail, Phone, ArrowRight, Clock, Shield, RefreshCw } from 'lucide-react'
...
const SERVICE_LABELS: Record<string, string> = {
  ...
  professional_drawings:  'Professional Drawings',
}
...
function GotYouInner() {
  ...
  if (paymentFailed) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-amber-50 flex flex-col items-center justify-center px-4 py-16">
        <div className="w-full max-w-lg">

          {/* Icon + heading */}
          <div className="text-center mb-8">
            <div className="mx-auto w-20 h-20 rounded-full bg-amber-100 flex items-center justify-center mb-6 shadow-sm ring-8 ring-amber-50">
            ...
            <p className="text-slate-500 text-base leading-relaxed">
              ...
              Your card has <span className="font-semibold">not been charged</span>.
            </p>
            {email && (
              <p className="text-slate-400 text-sm mt-2">
                A Kealee specialist will reach out to{' '}
            ...
            {!email && (
              <p className="text-slate-400 text-sm mt-2">
                A Kealee specialist will follow up to help you complete your order.
              </p>
            )}
          </div>
          ...
          <div className="bg-white rounded-2xl border border-amber-200 shadow-sm p-6 mb-4">
            <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-4">What to do next</p>
            <div className="space-y-4">
              {[
                { icon: RefreshCw,    text: 'Try again — go back and re-enter your payment details' },
                { icon: Mail,         text: 'Email us and we\'ll send a secure payment link directly' },
                { icon: Clock,        text: 'A Kealee specialist will follow up within a few hours' },
                { icon: Shield,       text: 'Your project details have been saved — nothing is lost' },
              ].map(({ icon: Icon, text }) => (
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <a
              href="mailto:hello@kealee.com?subject=Payment%20issue%20-%20help%20completing%20my%20order"
              className="flex-1 flex items-center justify-center gap-2 border border-amber-200 hover:bg-amber-50 text-slate-700 font-semibold px-6 py-3.5 rounded-xl transition"
            >
              Email us to pay
            </a>
          </div>
