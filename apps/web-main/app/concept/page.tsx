      ...
      <section className="py-16 bg-white">
        <Container width="lg">
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {PATHS.map(path => (
              <Link
                key={path.label}
                href={path.href}
                className="group flex flex-col rounded-2xl border border-gray-100 bg-white p-7 transition-all hover:-translate-y-1 hover:shadow-lg"
              >
                <div
          ...
          <p className="mt-8 text-center text-xs text-gray-400">
            Not sure which path to choose? <Link href="/contact" className="underline hover:text-gray-600">Talk to a human</Link>
          </p>
        </Container>
      </section>
    </>
