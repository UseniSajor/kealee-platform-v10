 ...
 * for projects / sales / revenue comparison in Command Center ops.
 */
...
/** Default estimated acquisition cost per converted lead (USD cents). */
export const DEFAULT_COST_PER_SALE_CENTS: Record<MarketingChannelBucket, number> = {
  organic: 0,
  paid_ads: 12_000, // ~$120 CPA placeholder until ad spend API wired
  marketing_saas: 3_500, // ~$35/lead amortized SaaS seat
  unknown: 0,
}
...
export interface ResolvedAttribution extends ChannelAttributionInput {
  utm: UTMParams
}
...
export function resolveAttributionFields(input: ChannelAttributionInput): ResolvedAttribution {
...
/**
 * Classify a lead into organic | paid_ads | marketing_saas | unknown.
 * Explicit metadata.marketingChannel wins when set at capture time.
 */
export function classifyMarketingChannel(input: ChannelAttributionInput): MarketingChannelBucket {
  const { utm } = resolveAttributionFields(input)
  const meta = (input.metadata ?? {}) as Record<string, unknown>
  const fd = (input.form_data ?? {}) as Record<string, unknown>
...
/** Set at lead capture from UTM + source (stored in metadata). */
export function marketingChannelFromUtm(utm: UTMParams, source?: string): MarketingChannelBucket {
  return classifyMarketingChannel({
    source,
    metadata: {
...
export interface ChannelBucketMetrics {
  ...
  label: string
  /** Intake leads (project inquiries). */
  projects: number
  leads: number
  /** Stripe-paid intakes. */
  sales: number
  conceptReady: number
  /** Sum of payment_amount (cents) for paid + concept_ready + processing. */
  revenueCents: number
  conversionRate: string
  /** Estimated acquisition spend (cents) for this bucket. */
  estimatedCostCents: number
  /** (revenue - cost) / cost as % string, or "—" if cost is 0. */
  roi: string
  revenueUsd: string
  costUsd: string
}
...
export interface ChannelComparisonOptions {
  ...
  /** Actual period spend (cents) — overrides CPA×sales when provided. */
...
function resolveBucketCost(
  ...
  if (actual !== undefined && actual > 0) {
    return { costCents: actual, usedActual: true }
  }
  if (bucket === 'organic' || bucket === 'unknown') {
    return { costCents: 0, usedActual: false }
  }
