...
function healthColor(m: MetricValue): string {
  if (m.isHealthy === undefined) return 'text-slate-500'
  return m.isHealthy ? 'text-emerald-600' : 'text-rose-600'
}

function healthBg(m: MetricValue): string {
  if (m.isHealthy === undefined) return 'bg-slate-50'
  return m.isHealthy ? 'bg-emerald-50' : 'bg-red-50'
}
...
function MetricCard({ m }: { m: MetricValue }) {
  ...
  return (
    <div className={`rounded-xl border border-slate-200 p-5 ${healthBg(m)}`}>
    ...
    <div className="rounded-xl border border-slate-200 bg-white p-5">
      <div className="flex items-start justify-between mb-3">
        ...
        <span className={`text-xs px-2 py-1 rounded-full font-medium ${
          region.launched ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-700 text-slate-500'
        }`}>
...
function FunnelChart({ stages }: { stages: Array<{ stage: string; count: number }> }) {
  if (!stages.length) return <p className="text-slate-400 text-sm">No onboarding data yet.</p>
      ...
      {stages.map(s => {
        ...
        return (
          <div key={s.stage} className="flex items-center gap-3">
            <span className="text-xs text-slate-500 w-36 shrink-0">{STAGE_LABELS[s.stage] ?? s.stage}</span>
            <div className="flex-1 h-6 bg-slate-100 rounded overflow-hidden">
              <div
                className={`h-6 rounded flex items-center px-2 transition-all ${
                  isRejected ? 'bg-rose-500' : 'bg-[#2ABFBF]/50'
                }`}
                style={{ width: `${pct}%`, minWidth: s.count > 0 ? '2rem' : '0' }}
              />
            </div>
            <span className="text-xs font-mono text-slate-600 w-8 text-right">{s.count}</span>
          </div>
        )
      })}
    </div>
...
function ConfigToggle({
  ...
  return (
    <div className="flex items-center justify-between py-3 border-b border-slate-100">
      <div className="pr-4">
        <p className="text-sm text-slate-900 font-mono">{item.key}</p>
        ...
        {isBool ? (
          <button
            ...
            className={`relative w-11 h-6 rounded-full transition-colors ${
              item.value ? 'bg-[#2ABFBF]' : 'bg-slate-300'
            }`}
          >
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#F8FAFC] flex items-center justify-center">
        <p className="text-slate-500 animate-pulse">Loading launch dashboard...</p>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900">
      ...
      <header className="border-b border-slate-200 px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold text-slate-900">Launch Dashboard</h1>
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              {data.regions.length === 0 && (
                <p className="col-span-3 text-slate-400 text-sm py-8 text-center">
            ...
            <div>
              ...
              <div className="mt-6 grid grid-cols-2 gap-3">
                <div className="bg-white rounded-xl p-4 text-center">
                  ...
                  <p className="text-xs text-slate-500 mt-1">Avg days to approval</p>
                </div>
                <div className="bg-white rounded-xl p-4 text-center">
                  ...
                  return (
                    <div key={item.label} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 h-7 bg-slate-100 rounded overflow-hidden">
                        <div
                          className="h-7 rounded flex items-center px-3 text-xs font-mono text-slate-900"
