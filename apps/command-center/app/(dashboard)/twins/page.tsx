  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="font-display text-2xl font-bold text-slate-900">Digital Twins</h1>
      ...
      {loading && (
        <div className="flex items-center justify-center py-20">
          <div className="text-center">
            ...
            <p className="text-sm" style={{ color: '#94A3B8' }}>Loading digital twins…</p>
          </div>
        </div>
      )}
            ...
            {[
              ...
              <div key={t.tier} className="rounded-xl border p-4"
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-sm font-bold text-slate-900">{t.tier} — {t.label}</p>
                    <p className="text-xs" style={{ color: '#94A3B8' }}>{t.count} twins · {t.kpis} KPIs</p>
                  </div>
                </div>
                <p className="mt-1 text-xs" style={{ color: '#94A3B8' }}>{t.desc}</p>
              </div>
            ))}
          </div>
          ...
          <div className="mb-6 flex flex-col gap-3 sm:flex-row">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full rounded-lg border py-2.5 pl-10 pr-4 text-sm text-slate-900 placeholder-slate-400 focus:outline-none"
                style={{ borderColor: '#E2E8F0', backgroundColor: '#FFFFFF' }}
              />
            </div>
            <div className="flex gap-2">
              {['all', 'healthy', 'warning', 'critical'].map((f) => (
                <button key={f} onClick={() => setFilter(f)}
                  ...
                  style={{
                    backgroundColor: filter === f ? 'rgba(42, 191, 191, 0.15)' : '#E2E8F0',
                    color: filter === f ? '#2ABFBF' : '#64748B',
                  }}>
                  {f}
                </button>
              ))}
            ...
            <div className="flex gap-2">
              {['all', 'L1', 'L2', 'L3'].map((t) => (
                <button key={t} onClick={() => setTierFilter(t)}
                  ...
                  style={{
                    backgroundColor: tierFilter === t ? 'rgba(139, 92, 246, 0.15)' : '#E2E8F0',
                    color: tierFilter === t ? (t === 'all' ? '#8B5CF6' : tierColors[t]) : '#64748B',
                  }}>
                  {t === 'all' ? 'All Tiers' : t}
                </button>
              ))}
                  ...
                  <div key={twin.id} className="rounded-xl border p-5"
                    ...
                    <div className="mb-3 flex items-start justify-between">
                      <div>
                        <div className="flex items-center gap-2 flex-wrap">
                          <h3 className="font-semibold text-slate-900 text-sm leading-snug">{twin.project}</h3>
                    ...
                    <div className="mb-3 grid grid-cols-3 gap-2 text-center">
                      <div className="rounded-lg p-2" style={{ backgroundColor: '#F8FAFC' }}>
                        <p className="text-lg font-bold text-slate-900">{twin.alerts}</p>
                        <p className="text-xs" style={{ color: '#94A3B8' }}>Alerts</p>
                      </div>
                      <div className="rounded-lg p-2" style={{ backgroundColor: '#F8FAFC' }}>
                        <p className="text-lg font-bold" style={{ color: tColor }}>{twin.kpiCount}</p>
                        <p className="text-xs" style={{ color: '#94A3B8' }}>KPIs</p>
                      </div>
                      <div className="rounded-lg p-2" style={{ backgroundColor: '#F8FAFC' }}>
                        <p className="text-lg font-bold text-slate-900">{twin.modules.length}</p>
                        <p className="text-xs" style={{ color: '#94A3B8' }}>Modules</p>
                      </div>
                    </div>
                    ...
                    <div className="mb-3 space-y-1">
                      <div className="flex justify-between text-xs">
                        <span style={{ color: '#94A3B8' }}>Budget Variance</span>
                      ...
                      <div className="flex justify-between text-xs">
                        ...
                        <span className="font-medium text-slate-900">{twin.kpis.completion_pct}%</span>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {twin.modules.map((mod) => (
                        <span key={mod} className="rounded px-1.5 py-0.5 text-xs"
                          style={{ backgroundColor: 'rgba(42, 191, 191, 0.1)', color: '#94A3B8' }}>
                          {OS_MODULES[mod] ?? mod}
                        </span>
                      ))}
                    </div>
