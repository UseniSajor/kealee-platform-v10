...
export const api = {
  ...
  projects: {
    ...
    budget: {
      get: async (projectId: string) => {
        const { data } = await apiClient.get<ApiResponse<{ budget: ProjectBudget }>>(`/pm/projects/${projectId}/pm/budget`)
        return data
      },
      update: async (projectId: string, budget: ProjectBudget) => {
        const { data } = await apiClient.put<ApiResponse<{ budget: ProjectBudget }>>(`/pm/projects/${projectId}/pm/budget`, budget)
        return data
      },
    },
    ...
    documents: {
      list: async (projectId: string) => {
        const { data } = await apiClient.get<ApiListResponse<DocumentItem>>(`/pm/projects/${projectId}/pm/documents`)
        return data
      },
    },
    ...
    photos: {
      list: async (projectId: string) => {
        const { data } = await apiClient.get<ApiListResponse<PhotoItem>>(`/pm/projects/${projectId}/pm/photos`)
        return data
      },
    },
  },
  ...
  permits: {
    scheduleInspection: async (projectId: string, input: PermitScheduleInspectionInput) => {
      const { data } = await apiClient.post<ApiResponse<{ inspectionId: string }>>(
        `/pm/projects/${projectId}/pm/inspections`,
        input
      )
      return data
    },
    uploadDocs: async (permitId: string, files: File[]) => {
      ...
      const { data } = await apiClient.post<DocumentUploadResponse>(`/pm/permits/${permitId}/pm/documents`, form, {
  ...
  documents: {
    ...
    upload: async (projectId: string, files: File[], meta?: { folder?: string }) => {
      ...
      const { data } = await apiClient.post<DocumentUploadResponse>(`/pm/projects/${projectId}/pm/documents`, form, {
        headers: { "Content-Type": "multipart/form-data" },
      })
      return data
    },
  ...
  photos: {
    ...
    upload: async (projectId: string, files: File[], meta?: { category?: string }) => {
      ...
      const { data } = await apiClient.post<ApiResponse<{ photos: PhotoItem[] }>>(`/pm/projects/${projectId}/pm/photos`, form, {
        headers: { "Content-Type": "multipart/form-data" },
      })
      return data
    },
  ...
  safety: {
    ...
    dashboard: async (projectId?: string) => {
      const { data } = await apiClient.get<any>("/pm/safety/pm/dashboard", { params: projectId ? { projectId } : undefined })
      return data
    },
  },
  ...
  team: {
    ...
    directory: async () => {
      const { data } = await apiClient.get<any>("/pm/team/pm/directory")
      return data
    },
    workload: async () => {
  ...
  pmProjects: {
    ...
    statusReport: async (id: string) => {
      const { data } = await apiClient.get<any>(`/pm/projects/${id}/pm/reports/status`)
      return data
    },
  },
  ...
  estimates: {
    ...
    sections: async (estimateId: string) => {
      const { data } = await apiClient.get<any>(`/estimation/pm/estimates/${estimateId}/sections`)
      return data
    },
    createSection: async (estimateId: string, input: any) => {
      const { data } = await apiClient.post<any>(`/estimation/pm/estimates/${estimateId}/sections`, input)
      return data
    },
    createCsiSections: async (estimateId: string) => {
      const { data } = await apiClient.post<any>(`/estimation/pm/estimates/${estimateId}/sections/csi`)
      return data
    },
    // Line items
    lineItems: async (estimateId: string, filters?: Record<string, any>) => {
      const { data } = await apiClient.get<any>(`/estimation/pm/estimates/${estimateId}/items`, { params: filters })
      return data
    },
    createLineItem: async (estimateId: string, input: any) => {
      const { data } = await apiClient.post<any>(`/estimation/pm/estimates/${estimateId}/items`, input)
      return data
    },
    bulkCreateLineItems: async (estimateId: string, items: any[]) => {
      const { data } = await apiClient.post<any>(`/estimation/pm/estimates/${estimateId}/items/bulk`, { items })
      return data
    },
    addFromAssembly: async (estimateId: string, input: any) => {
      const { data } = await apiClient.post<any>(`/estimation/pm/estimates/${estimateId}/items/from-assembly`, input)
      return data
    },
    // Revisions
    revisions: async (estimateId: string) => {
      const { data } = await apiClient.get<any>(`/estimation/pm/estimates/${estimateId}/revisions`)
      return data
    },
    createRevision: async (estimateId: string, input: any) => {
      const { data } = await apiClient.post<any>(`/estimation/pm/estimates/${estimateId}/revisions`, input)
      return data
    },
    metrics: async () => {
  ...
  reports: {
    ...
    schedule: async (projectId: string) => {
      const { data } = await apiClient.get<any>(`/pm/reports/pm/schedule/${projectId}`)
      return data
    },
    budget: async (projectId: string) => {
      const { data } = await apiClient.get<any>(`/pm/reports/pm/budget/${projectId}`)
      return data
    },
    safety: async (projectId: string) => {
      const { data } = await apiClient.get<any>(`/pm/reports/pm/safety/${projectId}`)
      return data
    },
  ...
  subscription: {
    ...
    getSubscription: async () => {
      const { data } = await apiClient.get<any>("/billing/software/pm/subscription")
      return data
    },
  ...
  docusign: {
    ...
    sendForSignature: async (contractId: string) => {
      const { data } = await apiClient.post<any>(`/docusign/pm/contracts/${contractId}/send-for-signature`)
      return data
    },
    getSignatureStatus: async (contractId: string) => {
      const { data } = await apiClient.get<any>(`/docusign/pm/contracts/${contractId}/signature-status`)
      return data
    },
  },
}
