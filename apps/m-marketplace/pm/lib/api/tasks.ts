/**
 * Task API Service
 * Type-safe API client for task management operations using Clerk Auth
 */
...
export class TaskApiService {
  ...
  static async startTimeTracking(taskId: string) {
    // Note: Backend route may need to be created (POST /pm/tasks/:id/pm/time-tracking/start)
    return this.fetchWithAuth(`/pm/tasks/${taskId}/pm/time-tracking/start`, {
      method: 'POST',
    });
  }

  static async stopTimeTracking(taskId: string) {
    // Note: Backend route may need to be created (POST /pm/tasks/:id/pm/time-tracking/stop)
    return this.fetchWithAuth(`/pm/tasks/${taskId}/pm/time-tracking/stop`, {
      method: 'POST',
    });
  }
  ...
  static async getTaskReport(params: {
    ...
    // Note: Backend route may need to be created (GET /pm/reports/pm/tasks)
  ...
  static async getTimeTrackingReport(params: {
    ...
    // Note: Backend route may need to be created (GET /pm/reports/pm/time-tracking)
    const query = new URLSearchParams({
      startDate: typeof params.startDate === 'string' ? params.startDate : params.startDate.toISOString(),
      endDate: typeof params.endDate === 'string' ? params.endDate : params.endDate.toISOString(),
    });
    ...
    return this.fetchWithAuth(`/pm/reports/pm/time-tracking?${query.toString()}`);
  }
