...
export const api = {
  ...
  generateTasksFromSOW: (data: {
    ...
    apiRequest<{ result: any }>("/pm/tasks/generate", {
      method: "POST",
      body: JSON.stringify(data),
    }),
  ...
  getSOPExecutions: (projectId: string) =>
    apiRequest<{ executions: SOPExecution[]; total: number }>(`/pm/sop/executions?projectId=${projectId}`),

  getSOPExecution: (id: string) =>
    apiRequest<{ execution: SOPExecution }>(`/pm/sop/executions/${id}`),

  startSOPExecution: (templateId: string, projectId: string) =>
    apiRequest<{ execution: SOPExecution }>("/pm/sop/executions", {
      method: "POST",
  ...
  completeSOPStep: (executionId: string, stepId: string, data?: { notes?: string; evidence?: unknown }) =>
    apiRequest<{ execution: SOPExecution }>(`/pm/sop/executions/${executionId}/steps/${stepId}/complete`, {
      method: "POST",
      body: JSON.stringify(data || {}),
    }),

  skipSOPStep: (executionId: string, stepId: string, reason: string) =>
    apiRequest<{ execution: SOPExecution }>(`/pm/sop/executions/${executionId}/steps/${stepId}/skip`, {
      method: "POST",
      body: JSON.stringify({ reason }),
    }),

  pauseSOPExecution: (id: string) =>
    apiRequest<{ execution: SOPExecution }>(`/pm/sop/executions/${id}/pause`, {
      method: "POST",
    }),

  resumeSOPExecution: (id: string) =>
    apiRequest<{ execution: SOPExecution }>(`/pm/sop/executions/${id}/resume`, {
      method: "POST",
    }),
  ...
  getSOPTemplates: (projectType?: string) => {
    ...
    const qs = params.toString()
    return apiRequest<{ templates: SOPTemplate[]; total: number }>(`/pm/sop/templates?${qs}`)
  },

  seedSOPTemplates: () =>
    apiRequest<{ template: SOPTemplate; message: string }>("/pm/sop/templates/seed", {
      method: "POST",
    }),
}
