...
export default function AutonomousActionsPage() {
  ...
  useEffect(() => {
    async function loadProjects() {
      try {
        const res = await fetch(`${API_BASE}/pm/projects`, { credentials: 'include' });
        if (res.ok) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [actionsRes, statsRes] = await Promise.all([
        fetch(`${API_BASE}/autonomy/pm/projects/${selectedProject}/actions?pageSize=50`, {
          credentials: 'include',
        }),
        fetch(`${API_BASE}/autonomy/pm/projects/${selectedProject}/stats?days=7`, {
          credentials: 'include',
        }),
      ]);
