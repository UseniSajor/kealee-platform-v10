...
import { useCreateChangeOrder } from "@pm/hooks/useChangeOrders"
      ...
      { ...form, lineItems, status, attachmentCount: attachments.length },
      {
        onSuccess: async (result: any) => {
          ...
          if (attachments.length > 0 && result?.data?.id) {
            try {
              const { api } = await import("@pm/lib/api")
              await api.changeOrders.uploadAttachments(result.data.id, attachments)
            } catch {
              // Change order created, attachments failed â€” non-blocking
              console.error("Attachment upload failed")
            }
          }
          router.push("/pm/change-orders")
        },
      }
    )
  }
        ...
        <CardContent className="space-y-4">
          ...
          >
            ...
            <p className="text-xs mt-1">
              Proposals, sketches, photos, reports â€” PDF, images, DWG up to 50 MB
            </p>
          </div>
