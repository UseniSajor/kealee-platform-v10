...
export async function middleware(request: NextRequest) {
  ...
  // Protected routes
  const protectedPaths = ['/owner/dashboard', '/owner/projects', '/owner/account', '/settings', '/owner/approvals', '/owner/reports'];
  ...
  const authPaths = ['/owner/login', '/owner/signup'];
  ...
  if (isProtectedPath && session) {
    ...
    if (!user || user.status !== 'ACTIVE') {
      const redirectUrl = request.nextUrl.clone();
      redirectUrl.pathname = '/owner/unauthorized';
      return NextResponse.redirect(redirectUrl);
    }
