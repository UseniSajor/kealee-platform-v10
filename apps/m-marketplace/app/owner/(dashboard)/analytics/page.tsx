...
export default function ClientAnalyticsPage() {
  ...
  useEffect(() => {
    async function loadProjects() {
      try {
        const res = await fetch(`${API_BASE}/owner/projects?limit=50`, { credentials: 'include' })
        if (res.ok) {
  ...
  useEffect(() => {
    ...
    async function loadBenchmark() {
      try {
        const res = await fetch(`${API_BASE}/owner/analytics/owner/dashboard/project/${selectedProjectId}`, {
          credentials: 'include',
        })
