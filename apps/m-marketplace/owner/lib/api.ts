...
export const api = {
  ...
  createProject: (data: CreateProjectInput) =>
    apiRequest<{ project: ProjectSummary }>('/owner/projects', { method: 'POST', body: data }),
  updateProject: (id: string, data: UpdateProjectInput) =>
    apiRequest<{ project: ProjectSummary }>(`/owner/projects/${id}`, { method: 'PATCH', body: data }),
  getProject: (id: string) => apiRequest<{ project: Record<string, unknown> }>(`/owner/projects/${id}`),
  listMyProjects: () => apiRequest<{ projects: ProjectSummary[] }>('/owner/projects'),
  addProjectMember: (id: string, data: { userId: string; role: ProjectMemberRole }) =>
    apiRequest<{ member: Record<string, unknown> }>(`/owner/projects/${id}/members`, { method: 'POST', body: data }),
  ...
  generateReadiness: (projectId: string) =>
    apiRequest<{ items: ReadinessItem[] }>(`/readiness/owner/projects/${projectId}/generate`, { method: 'POST' }),
  listReadiness: (projectId: string) =>
    apiRequest<{ items: ReadinessItem[] }>(`/readiness/owner/projects/${projectId}/items`),
  getReadinessCompletion: (projectId: string) =>
    ...
    }>(`/readiness/owner/projects/${projectId}/completion`),
  ...
  bulkCompleteReadinessItems: (projectId: string, itemIds: string[], reason?: string) =>
    apiRequest<{ items: ReadinessItem[] }>(`/readiness/owner/projects/${projectId}/bulk-complete`, {
      method: 'POST',
      body: { itemIds, reason },
    }),
  ...
  getContractTemplates: (params?: { orgId?: string; activeOnly?: boolean }) => {
    ...
    return apiRequest<{ templates: ContractTemplate[] }>(`/owner/contracts/templates${suffix}`)
  },
  previewContractTemplate: (templateId: string, projectId?: string, variables?: Record<string, string>) =>
    ...
    }>(`/owner/contracts/templates/${templateId}/preview`, { method: 'POST', body: { projectId, variables } }),
  ...
  }) => apiRequest<{ contract: Contract }>(`/owner/contracts`, { method: 'POST', body: data }),
  getContract: (id: string) => apiRequest<{ contract: Contract }>(`/owner/contracts/${id}`),
  listProjectContracts: (projectId: string) => apiRequest<{ contracts: Contract[] }>(`/owner/contracts/owner/projects/${projectId}`),
  updateContract: (id: string, data: { contractorId?: string | null; terms?: string | null; status?: string }) =>
    apiRequest<{ contract: Contract }>(`/owner/contracts/${id}`, { method: 'PATCH', body: data }),
  addMilestone: (contractId: string, data: { name: string; description?: string | null; amount: number }) =>
    apiRequest<{ milestone: Milestone }>(`/owner/contracts/${contractId}/milestones`, { method: 'POST', body: data }),
  updateMilestone: (milestoneId: string, data: { name?: string; description?: string | null; amount?: number }) =>
    apiRequest<{ milestone: Milestone }>(`/owner/contracts/milestones/${milestoneId}`, { method: 'PATCH', body: data }),
  deleteMilestone: (milestoneId: string) =>
    apiRequest<{ success: boolean }>(`/owner/contracts/milestones/${milestoneId}`, { method: 'DELETE' }),
  ...
  sendContractForSignature: (contractId: string) =>
    apiRequest<{ envelopeId: string; recipientViewUrl?: string }>(`/docusign/owner/contracts/${contractId}/send-for-signature`, {
      method: 'POST',
    }),
  getContractSignatureStatus: (contractId: string) =>
    apiRequest<{
      ...
      message?: string
    }>(`/docusign/owner/contracts/${contractId}/signature-status`),
  downloadSignedContract: async (contractId: string) => {
    const token = await getAuthToken()
    return fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/docusign/owner/contracts/${contractId}/signed-document`, {
      headers: {
  ...
  getContractAuditTrail: (contractId: string) =>
    ...
    }>(`/owner/contracts/${contractId}/audit-trail`),
  cancelContract: (contractId: string, reason: string) =>
    apiRequest<{ contract: Contract }>(`/owner/contracts/${contractId}/cancel`, { method: 'POST', body: { reason } }),
  archiveContract: (contractId: string) =>
    apiRequest<{ contract: Contract }>(`/owner/contracts/${contractId}/archive`, { method: 'POST' }),
  ...
  validateContractCompliance: (contractId: string) =>
    ...
    }>(`/owner/contracts/${contractId}/compliance`),
  ...
  getContractMilestones: (contractId: string) =>
    ...
    }>(`/milestones/owner/contracts/${contractId}/milestones`),
  ...
  getEscrowAgreement: (projectId: string) =>
    ...
    }>(`/owner/payments/owner/projects/${projectId}/escrow`),
  canReleasePayment: (milestoneId: string) =>
    apiRequest<{
      canRelease: boolean
      reasons: string[]
    }>(`/owner/payments/milestones/${milestoneId}/can-release`),
  releasePayment: (milestoneId: string, options?: { skipHoldback?: boolean; notes?: string }) =>
    apiRequest<{
      ...
      balanceAfter: number
    }>(`/owner/payments/milestones/${milestoneId}/release-payment`, {
      method: 'POST',
      body: options || {},
    }),
  getPaymentHistory: (projectId: string) =>
    ...
    }>(`/owner/payments/owner/projects/${projectId}/owner/payments`),
  ...
  listProjectDisputes: (projectId: string) =>
    ...
    }>(`/disputes/owner/projects/${projectId}`),
  ...
  checkPermitCompliance: (projectId: string, milestoneId?: string) =>
    ...
    }>(`/permits/owner/projects/${projectId}/compliance${milestoneId ? `?milestoneId=${milestoneId}` : ''}`),
  getPermitStatusSummary: (projectId: string) =>
    ...
    }>(`/permits/owner/projects/${projectId}/status`),
  ...
  getCloseoutChecklist: (projectId: string) =>
    ...
    }>(`/closeout/owner/projects/${projectId}/checklist`),
  ...
  completeCloseout: (projectId: string) =>
    ...
    }>(`/closeout/owner/projects/${projectId}/complete`, {
      method: 'POST',
    }),
  getPunchListItems: (projectId: string) =>
    ...
    }>(`/closeout/owner/projects/${projectId}/punch-list`),
  ...
  generateHandoffPackage: (projectId: string) =>
    ...
    }>(`/handoff/owner/projects/${projectId}/generate`, {
      method: 'POST',
    }),
  getHandoffPackage: (projectId: string) =>
    ...
    }>(`/handoff/owner/projects/${projectId}`),
  deliverHandoffPackage: (projectId: string) =>
    ...
    }>(`/handoff/owner/projects/${projectId}/deliver`, {
      method: 'POST',
    }),
  recordHandoffDownload: (projectId: string) =>
    ...
    }>(`/handoff/owner/projects/${projectId}/download`, {
      method: 'POST',
    }),
