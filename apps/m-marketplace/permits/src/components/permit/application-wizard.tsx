...
import { applicationStorageService } from '@permits/src/services/permit-application/application-storage';
...
export function PermitApplicationWizard({draftId, onSave}: PermitApplicationWizardProps) {
  ...
  const handleSubmit = async (data: WizardFormData) => {
    ...
    try {
      ...
      window.location.href = `/permits/permits/status/${result.id || result.application?.id}`;
    } catch (error) {
