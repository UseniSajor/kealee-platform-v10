...
import { ArrowLeft, ArrowUpRight, Building2, CheckCircle2, Clock3, Download, Globe2, LifeBuoy, Play, Plus, RefreshCw, ShieldCheck, Trash2 } from 'lucide-react'
...
import { extractCollection, extractObject, whiteLabelApi, type WhiteLabelBranding, type WhiteLabelDataRequest, type WhiteLabelDeployment, type WhiteLabelDomain, type WhiteLabelEvaluation, type WhiteLabelPlan, type WhiteLabelProduct, type WhiteLabelSupportSession, type WhiteLabelTenant, type WhiteLabelUsage } from '@/lib/white-label-api'

type Tab = 'overview' | 'branding' | 'domains' | 'products' | 'modules' | 'plan' | 'usage' | 'deployment' | 'evaluations' | 'support' | 'data'
const tabs: Array<{ key: Tab; label: string }> = [
  ...
  { key: 'usage', label: 'Usage' }, { key: 'deployment', label: 'Deployment' }, { key: 'evaluations', label: 'Evaluations' }, { key: 'support', label: 'Support access' }, { key: 'data', label: 'Data lifecycle' },
]
...
export default function WhiteLabelTenantPage() {
  ...
  const [dataRequests, setDataRequests] = useState<WhiteLabelDataRequest[]>([])
  const [dataRequestType, setDataRequestType] = useState<WhiteLabelDataRequest['requestType']>('DATA_EXPORT')
  ...
  const loadPanel = useCallback(async (selected: Tab) => {
    ...
    try {
      ...
      if (selected === 'data') setDataRequests((await whiteLabelApi.getDataRequests(orgId)).dataRequests)
    } catch (cause) {
  ...
  async function downloadDataExport(requestId: string) {
    await mutate(async () => {
      const result = await whiteLabelApi.getDataExport(orgId, requestId)
      const url = URL.createObjectURL(new Blob([JSON.stringify(result.payload, null, 2)], { type: 'application/json' }))
      const link = document.createElement('a')
      link.href = url
      link.download = `kealee-tenant-${orgId}-${requestId}.json`
      link.click()
      URL.revokeObjectURL(url)
    }, 'Tenant export downloaded.', false)
  }
  ...
  return <ProtectedRoute><AppLayout><div className="mx-auto max-w-7xl p-6 sm:p-8">
    ...
    {loading && !tenant ? <div className="space-y-4" aria-label="Loading tenant"><div className="h-14 w-1/2 animate-pulse rounded-lg bg-slate-100" /><div className="h-64 animate-pulse rounded-xl bg-slate-100" /></div> : error || !tenant ? <div role="alert" className="rounded-lg border border-red-200 bg-red-50 p-5 text-sm text-red-800">{error || 'Tenant unavailable.'}<Button className="ml-3" variant="outline" onClick={() => void loadTenant()}>Retry</Button></div> : <>
      ...
      {panelLoading && tab !== 'branding' && tab !== 'overview' ? <div className="h-40 animate-pulse rounded-xl bg-slate-100" /> : <div className="space-y-5">
        ...
        {tab === 'data' && <Panel title="Data lifecycle" description="Export, retention, and control-plane deletion requests are tenant-scoped, approval-gated, and audited. Homeowner/project records are handled through their separate project lifecycle.">
          <form onSubmit={event => { event.preventDefault(); if (dataRequestReason.trim().length >= 10) void mutate(async () => { await whiteLabelApi.createDataRequest(orgId, { requestType: dataRequestType, reason: dataRequestReason.trim(), requestedRetentionDays: dataRequestType === 'RETENTION_CHANGE' ? Number(retentionDays) : undefined }); setDataRequestReason('') }, 'Data lifecycle request created.') }} className="mb-5 grid gap-3 md:grid-cols-[auto_1fr_auto_auto]">
            <select aria-label="Data request type" className="h-9 rounded-md border bg-white px-3 text-sm" value={dataRequestType} onChange={event => setDataRequestType(event.target.value as WhiteLabelDataRequest['requestType'])}><option value="DATA_EXPORT">Control-plane export</option><option value="RETENTION_CHANGE">Retention change</option><option value="DATA_DELETION">Control-plane deletion</option></select>
            <Input aria-label="Data request reason" placeholder="Reason (at least 10 characters)" value={dataRequestReason} onChange={event => setDataRequestReason(event.target.value)} minLength={10} required />
            {dataRequestType === 'RETENTION_CHANGE' ? <Input aria-label="Retention days" type="number" min={1} max={3650} value={retentionDays} onChange={event => setRetentionDays(event.target.value)} /> : <span />}
            <Button type="submit" disabled={busy || dataRequestReason.trim().length < 10}><Plus className="h-4 w-4" />Create request</Button>
          </form>
          <div className="mb-5 rounded-lg border border-amber-200 bg-amber-50 p-4"><p className="text-sm font-medium text-amber-950">Destructive confirmation</p><p className="mt-1 text-xs text-amber-800">Deletion removes the white-label control plane and archives the professional organization. It does not delete homeowner or operational project records.</p><Input className="mt-3" aria-label="Deletion confirmation" placeholder={`DELETE WHITE-LABEL TENANT ${orgId}`} value={deletionConfirmation} onChange={event => setDeletionConfirmation(event.target.value)} /></div>
          {dataRequests.length === 0 ? <Empty message="No data lifecycle requests have been recorded." /> : <div className="divide-y rounded-lg border">{dataRequests.map(item => <div key={item.id} className="flex flex-wrap items-center gap-3 p-4"><div className="min-w-64 flex-1"><p className="font-medium">{item.requestType.replace(/_/g, ' ')}</p><p className="text-xs text-slate-500">{item.status} · {new Date(item.createdAt).toLocaleString()}{item.requestedRetentionDays ? ` · ${item.requestedRetentionDays} days` : ''}</p>{item.failureReason && <p className="mt-1 text-xs text-red-700">{item.failureReason}</p>}</div>
            {item.status === 'REQUESTED' && <><Button size="sm" disabled={busy || dataRequestReason.trim().length < 10} onClick={() => void mutate(() => whiteLabelApi.reviewDataRequest(orgId, item.id, 'APPROVE', dataRequestReason.trim()), 'Request approved.')}>Approve</Button><Button size="sm" variant="outline" disabled={busy || dataRequestReason.trim().length < 10} onClick={() => void mutate(() => whiteLabelApi.reviewDataRequest(orgId, item.id, 'REJECT', dataRequestReason.trim()), 'Request rejected.')}>Reject</Button></>}
            {item.status === 'APPROVED' && item.requestType === 'DATA_EXPORT' && <Button size="sm" disabled={busy} onClick={() => void mutate(() => whiteLabelApi.executeDataExport(orgId, item.id), 'Export generated.')}><Play className="h-4 w-4" />Generate</Button>}
            {item.status === 'APPROVED' && item.requestType === 'RETENTION_CHANGE' && <Button size="sm" disabled={busy} onClick={() => void mutate(() => whiteLabelApi.executeRetentionChange(orgId, item.id), 'Retention updated.')}><Play className="h-4 w-4" />Apply</Button>}
            {item.status === 'APPROVED' && item.requestType === 'DATA_DELETION' && <Button size="sm" variant="destructive" disabled={busy || deletionConfirmation !== `DELETE WHITE-LABEL TENANT ${orgId}` || dataRequestReason.trim().length < 10} onClick={() => void mutate(() => whiteLabelApi.executeDataDeletion(orgId, item.id, deletionConfirmation, dataRequestReason.trim()), 'White-label control plane deleted.')}><Trash2 className="h-4 w-4" />Delete control plane</Button>}
            {item.status === 'COMPLETED' && item.requestType === 'DATA_EXPORT' && item.exportExpiresAt && <Button size="sm" variant="outline" disabled={busy} onClick={() => void downloadDataExport(item.id)}><Download className="h-4 w-4" />Download</Button>}
            {['REQUESTED', 'APPROVED'].includes(item.status) && <Button size="sm" variant="ghost" disabled={busy || dataRequestReason.trim().length < 10} onClick={() => void mutate(() => whiteLabelApi.cancelDataRequest(orgId, item.id, dataRequestReason.trim()), 'Request cancelled.')}>Cancel</Button>}
          </div>)}</div>}
        </Panel>}
        {tab === 'support' && <Panel title="Audited support access" description="Every tenant support session must have a reason and is recorded with its actor and expiration."><form onSubmit={event => { event.preventDefault(); if (supportReason.trim().length >= 10) void mutate(async () => { await whiteLabelApi.requestSupportAccess(orgId, supportReason.trim(), ['TENANT_READ'], new Date(Date.now() + Number(supportHours) * 3600000).toISOString()); setSupportReason('') }, 'Read-only support access requested.') }} className="mb-5 grid gap-3 sm:grid-cols-[1fr_auto_auto]"><Input aria-label="Reason for support access" placeholder="Reason for request or revocation (at least 10 characters)" value={supportReason} onChange={event => setSupportReason(event.target.value)} required minLength={10} /><select aria-label="Access duration" className="h-9 rounded-md border bg-white px-3 text-sm" value={supportHours} onChange={event => setSupportHours(event.target.value)}><option value="1">1 hour</option><option value="4">4 hours</option><option value="24">24 hours</option></select><Button disabled={busy} type="submit"><LifeBuoy className="h-4 w-4" />Request read access</Button></form>{supportSessions.length === 0 ? <Empty message="No support access sessions have been recorded." /> : <div className="divide-y rounded-lg border">{supportSessions.map(session => <div key={session.id} className="flex flex-wrap items-center gap-3 p-4"><Clock3 className="h-4 w-4 text-slate-500" /><div className="flex-1"><p className="font-medium">{session.reason}</p><p className="text-xs text-slate-500">{session.status || 'Status unavailable'}{session.actorId ? ` · ${session.actorId}` : ''}{session.expiresAt ? ` · Expires ${new Date(session.expiresAt).toLocaleString()}` : ''}</p></div>{!['REVOKED', 'DENIED', 'EXPIRED'].includes((session.status ?? '').toUpperCase()) && <Button size="sm" variant="outline" disabled={busy || supportReason.trim().length < 10} onClick={() => void mutate(() => whiteLabelApi.closeSupportAccess(orgId, session.id, supportReason.trim()), 'Support access revoked.')}>Revoke</Button>}</div>)}</div>}</Panel>}
      </div>}
    </>}
