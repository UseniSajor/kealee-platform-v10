...
async function getConceptPackage(intakeId: string) {
  ...
  for (const path of [`/concepts/package/${intakeId}`, `/api/v1/concepts/package/${intakeId}`]) {
    try {
      ...
      // Preserve legacy portal behavior if the API is not running locally.
    }
  }
  return null
}
...
export async function GET() {
  ...
  const packageRows = await Promise.all((data ?? []).map((row: any) => getConceptPackage(row.id)))
  const enrichedData = (data ?? []).map((row: any, index: number) => {
    const pkg = packageRows[index]
    if (!pkg) return row
    ...
    return {
      ...row,
      status: row.status === 'concept_ready' ? row.status : 'concept_ready',
      form_data: {
        ...formData,
        conceptOutput: {
          ...(legacy ?? {}),
          conceptPackageId: pkg.id,
          packageJson: pkg.packageJson ?? legacy?.packageJson,
          pdfUrl: pkg.pdfUrl ?? legacy?.pdfUrl,
          floorplanUrl: pkg.floorplanUrl ?? legacy?.floorplanUrl,
          renderImageUrls: pkg.renderImageUrls ?? legacy?.renderImageUrls,
          generatedAt: pkg.completedAt ?? pkg.createdAt ?? legacy?.generatedAt,
        },
      },
    }
  })

  const ownedProducts = ownedProductsFromRows(enrichedData)

  // ── Batch-fetch service_chain_gates for lifecycle stage ─────────────────────
  const intakeIds = enrichedData.map((r: any) => r.id as string)
  ...
  const deliverables = enrichedData.map((row: any) => {
